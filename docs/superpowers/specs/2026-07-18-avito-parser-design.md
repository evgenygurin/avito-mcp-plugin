# Дизайн: полнофункциональный парсер каталога Avito в MCP-плагине

**Дата:** 2026-07-18
**Статус:** одобрен пользователем, готов к плану реализации
**Ветка:** `feat/avito-parser-core`

## 1. Цель и контекст

Превратить `avito-mcp-plugin` в полнофункциональный Avito-тулкит для AI-агентов —
самостоятельный парсер публичного каталога Avito, поставляемый как **MCP-тулзы +
skills** (без GUI). Текущая официально-API-центричная реализация («Линия A»)
удаляется целиком; сервер строится с нуля вокруг собственного движка парсинга,
обёрнутого в MCP.

Проверенный метод парсинга воспроизведён и **валидирован живьём** 2026-07-18:
spfa-куки → ротация IP до чистого → curl_cffi (`impersonate`) + follow SSR-редиректа
→ извлечение `loaderData.data.catalog` → 50 объявлений (НН/квартиры). Артефакты:
`robust_run.py`, `page.html`, `result/listings.json` (станут фикстурами).

### Ключевые решения (согласованы)

1. **Legal-позиция:** guardrails-документация убирается. Полный фичесет парсинга по
...
2. **Архитектура:** agent-driven stateless-тулзы + лёгкое sqlite-состояние.
   ...
   в сервере.
3. **Разворот к парсингу:** удалить все существующие тулзы (официальный API +
   `ping`); внутренняя структура сервера строится под собственный движок парсинга.
...
> кабинетом — легальнейшая и рабочая часть. В целевом фичесете парсинга такого
> функционала не предусмотрено → удаляем осознанно.
...
несёт весь движок парсинга. Внутренняя структура `server/src/avito_mcp_server/`
отражает модульную раскладку движка парсинга:
...
### Улучшение retry-логики (валидировано живьём)

HTTP-клиент сохраняет проверенный подход (curl_cffi + `impersonate`), но **заменяет
...
Это чинит типичный дефект наивной retry-логики (`block_threshold`+
`max_count_of_retry` дают одну ротацию и сдачу), из-за которого такие парсеры не
пробивают смешанный IP-пул.

## 4. Инвентарь MCP-тулз (7 шт., полный фичесет парсинга)

Фильтры и `parse_views` — **параметры**, не отдельные тулзы. Держит нас глубоко под
лимитом Anthropic «< 20 тулз». `parse_phone` **исключён** (ПДн третьих лиц).

| # | Тулза | Назначение | Ключевые параметры |
|---|---|---|---|
| 1 | `search_listings` | поиск объявлений по каталогу | `url_or_query`, `region`, `pages`, `include_keywords`, `exclude_keywords`, `seller_blacklist`, `price_min/max`, `geo`, `max_age`, `parse_views` |
...
Модель `models.py`, упрощённая до фактов + опций:
