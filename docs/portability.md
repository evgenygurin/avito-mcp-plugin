...
Слой тулз — полнофункциональный парсер каталога Avito (свой движок парсинга): `search_listings`,
`get_listing`, `scan_new_listings`, `check_proxy_health`, `send_notification`,
`export_listings`, `get_price_history` (полный список параметров —
[`mcp-server.md`](mcp-server.md)). Ниже — как один и тот же набор тулз подключить
к разным агентам через их конфиг MCP; сами имена и параметры тулз от конфига
не зависят.
