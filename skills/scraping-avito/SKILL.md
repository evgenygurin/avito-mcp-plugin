---
name: scraping-avito
description: Use when scraping public Avito catalog/listings or building/debugging the parsing engine — choosing a cookie provider (spfa default / own / playwright), hitting 403 «проблема с IP» or 429 rate limits, needing IP rotate-until-clean, following the SSR redirect to a canonical category URL, or extracting loaderData JSON from a blank JS page. Covers RU proxy hygiene and the cookies→rotate→HTTP hybrid.
---

# Scraping Avito (движок парсинга)

> How-to движка. Полная техдока (инструменты, прокси, БД) — в `docs/avito-scraping.md`.
...
и `parse_views` — тоже параметры, не отдельные шаги.
...
- Смапь в модель `Listing` (факты: `id`, `title`, `price`, `url`, `address`,
  `params`, `seller_id`, `is_promotion`, `published_at`; `views` — если `parse_views`).
...
## Тулзы (статус 🔜 план — код ещё не написан)

Движок обёрнут в MCP-тулзы `search_listings`, `get_listing`, `check_proxy_health`
(парсинг) и `scan_new_listings`, `get_price_history`, `export_listings`,
`send_notification` (состояние/сайд-эффекты). Все в статусе плана; код —
в `server/src/avito_mcp_server/` (`cookies/`, `proxies/`, `http/`, `parser.py`,
`filters/`, `tools/`).

## Related

- `docs/avito-scraping.md` — полная техдока движка (инструменты, прокси, БД)
