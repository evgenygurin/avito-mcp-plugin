...
## Эскалация прокси (`MpsApiProxy`, опционально) — когда rotate-until-clean не спасает

rotate-until-clean лечит смешанный пул (часть IP чистая, часть — 403). Но
бывает, что **прожжена вся подсеть** региона/оператора целиком (эмпирика:
Билайн/Москва — 18/18 блоков; проверено `check_spam` через API mpsapi.com —
IP в блэклистах StopForumSpam) — тогда ротация IP внутри неё не помогает,
нужна смена физической точки (регион/оператор), раньше — только вручную через
кабинет mobileproxy.space.

Если заданы `AVITO_MPS_API_TOKEN` + `AVITO_PROXY_ID` (провайдер mobileproxy.space,
API mpsapi.com), фабрика прокси собирает `MpsApiProxy` вместо `MobileProxy`.
`HttpClient.get()` при исчерпании обычного круга ротаций (rotate-until-clean)
сам вызывает `proxy.escalate()` **один раз за вызов**: запрашивает
`get_geo_operator_list`, выбирает не-московскую точку с наибольшим числом
свободных портов у настроенного оператора (`AVITO_MPS_OPERATOR`, дефолт
`megafone`), переключает через `change_equipment` и повторяет полный круг
попыток на новой точке. Не повторяет уже испробованные в этом прогоне точки.

Без этих двух переменных ничего не меняется — используется обычный
`MobileProxy` (только `rotate()`), как и раньше.
...
| `AVITO_MPS_API_TOKEN` | Bearer-токен API mobileproxy.space — включает автоэскалацию региона/оператора |
| `AVITO_PROXY_ID` | ID прокси в кабинете mobileproxy.space (нужен вместе с токеном) |
