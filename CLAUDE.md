...
| `AVITO_MAX_ROTATE_ATTEMPTS` | лимит перебираемых комбинаций IP/кук (дефолт 12) |
| `AVITO_DIRECT_FIRST` | пробовать прямое соединение перед прокси (дефолт 1) |
...
- **Прокси — не синоним «надёжнее».** Замер 2026-07-20: прямое соединение с
  куками spfa отдаёт каталог за 0.63 с, а купленная мобильная подсеть — 403 на
  любом выходном IP и любом отпечатке (Qrator банит подсеть целиком, ротация
  меняет адрес, но не репутацию). Поэтому `build_proxy` по умолчанию строит
  `ChainProxy([NoProxy(), <настроенный прокси>])`: сначала напрямую, прокси —
  фоллбэк. Смена звена мгновенна и идёт БЕЗ backoff (`rotation_was_instant`),
  дорогая ротация IP — только когда звенья кончились. Отключается
  `AVITO_DIRECT_FIRST=0`.
